...
class WPFileUploadHandler
{
    ...
    public function save_file(string $url, bool $overwrite = false)
    {
        ...
        $upload_location = self::getUploadFolder($winner_post_id);

        if ($this->file_exists($upload_location, $url) && $overwrite === false) {
            $this->file_path = self::getUploadFolder($winner_post_id, "uri").basename($url);
        ...
        $this->file_path = self::getUploadFolder($winner_post_id, "uri").$file_array['name'];
     ...
     * @throws \Exception
     */
    public function upload_attachment(string $url, string $desc, string $attachment_type)
    {
